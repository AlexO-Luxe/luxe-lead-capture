...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (duplicateOf) console.log(`Duplicate detected (${duplicateOf.matchCount}/4) — existing lead ID:`, duplicateOf.id);
  } catch(err) {
...
async function sendTeamNotification(p, mondayId, mondayError, duplicateOf, submitterIp) {
  ...
  // Monday error banner — shown when push failed. When the write fails this
  // email is the ONLY copy of the lead's attribution, so include every
  // tracking field for manual entry (parity with the Student Luxe handler).
