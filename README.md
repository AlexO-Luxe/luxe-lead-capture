...
| `api/_manual-todos.js` | Standing manual jobs (paste a script into Squarespace, retire a Jotform) that sit at the top of the weekly PPC review to-do card until cleared. Seeded in code, done-state in KV `todo:manual:done`, cleared with `/api/pmax-review?secret=<CRON_SECRET>&todoDone=<id>` |
| `api/_booker-profile.js` | Booker profile for the weekly PPC review: every booked lead's Monday fields (country, nationality, city, university, budget band, apartment type, device, source, channel, first channel, campaign, first campaign, search term, landing page) compared against all leads in the window (default 180 days, `?profileDays=`). Renders a to-do card of concrete Ads actions (bid up countries with lift over 1.3x, add bookers' search terms as search themes, landing pages, universities, dead budget bands, budget split) and the evidence card behind it. Each action needs two bookings except search terms |
| `api/pmax-review.js` | Weekly cron (Mon 07:30 UTC). Weekly PPC review email: booker profile to-do list and evidence, then the Performance Max review: campaign scoreboard (Google cost and Step 1 to 4 against Monday leads, junk and bookings), asset groups spending with no Step 1, and Google search categories clicking with no conversions (negative keyword candidates). Manual: `?secret=<CRON_SECRET>&days=14&minSpend=40&minClicks=30&dryRun=1`. `?section=1` stashes the card for the combined weekly report, `?cached=1` serves it |
...
- **Performance Max never supplies a keyword.** A keyword shown on a Performance Max lead is a Search keyword carried over from an earlier click via the 90 day `sl_term` cookie. Both tracking snippets now clear keyword-level last-touch cookies (adgroup, term, matchtype, and the other click id types) whenever a new Google click lands without them, and the team email labels any remaining carry-over as "Earlier search term (first click, campaign)" via `searchTermRows`. Google offers no per-click query for Performance Max; the only source is the campaign-level Search Terms Insights report in the Ads UI.
