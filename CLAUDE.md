...
- `api/_landing-check.js` — follows every Step 1 upload through to a recorded conversion (`checkLandingWindow`, rolling 7 days, each day judged 3 days after upload so Google has settled). Feeds the Conversion follow up section of the daily digest with a running verified/waiting/missing total. Reconciles uploads against the conversions Google actually recorded (`all_conversions_by_conversion_date`, judged 3 days back). "Uploaded" only means the API accepted the request; Google validates asynchronously and can drop an event silently. Uploads with no click id are excluded from the rate, they carry a hashed email only and mostly cannot match a click. Also owns retraction reporting: `RETRACTION_OUTCOMES` + `classifyRetraction` + `retractionLedger` (30 day running total by outcome), and `missingLeads` (KV `gads:missing`, junk leads Google could not find when asked to remove them). Keep landing and retraction in separate digest cards, they are opposite directions of travel and reading as one card is what made the email confusing.
- `api/_audience.js` / `api/audience-sync.js` — two self-feeding Customer Match lists: All Bookings (9451577693) and High Value Bookers £5k+ (9467837898, single booking value at or over £5,000). Real-time add on booking confirmation plus nightly full-history sweep 05:45 UTC; newsletter opt-outs (`cm:optout`) excluded from both.
